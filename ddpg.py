...
class DDPG:
    ...
    def add_experience(self, observation_1, observation_2, action, reward):
        ...
        print(self.observation_1)
        self.observation_2 = observation_2
        self.action = action
        self.reward = reward
#        self.done = done
        self.replay_memory.append((self.observation_1, self.observation_2, self.action, self.reward))
    ...
    def minibatches(self):
        ...
        self.state_t_batch = np.reshape(self.state_t_batch,[BATCH_SIZE, self.num_states])
        #state t+1        
        self.state_t_1_batch = np.array([item[1] for item in batch])
        self.state_t_1_batch = np.reshape(self.state_t_1_batch,[BATCH_SIZE, self.num_states])
